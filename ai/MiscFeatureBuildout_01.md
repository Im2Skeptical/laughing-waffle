...
If not please have them roll as if they were items. The rot when created will need to be added to a separate system pool. 

---

Granary hubStructure needs to improves the rot chance for items stored inside. We will do this using the tier system; using a passive on the structure tied to a tag called 'canPreserve' that upgrades the perishability of system pool stored grain by one tier

---

We need to expand the functionality of the distributor tags / or at least how pawns can use it. The design goal is for pawns to be able to satisfy eating intents from a storehouse that is a distributor and that contains edible items.
